...
if($_SESSION['nivel']==0){
    if(($total_puntos==1) OR ($cedula_usuario=='')){
        $query="SELECT * FROM punto_cuenta, usuarios WHERE date_part('year',punto_cuenta.fecha_punto)= '$year' AND punto_cuenta.condicion=$condicion AND punto_cuenta.cedula_usuario = usuarios.cedula_usuario  AND punto_cuenta.condicion<>0 order by punto_cuenta.id_punto DESC LIMIT 10 OFFSET '$page'";
        $result = pg_query($query)or die(pg_last_error());

        $query2="SELECT * FROM punto_cuenta, usuarios WHERE date_part('year',punto_cuenta.fecha_punto)= '$year' AND punto_cuenta.condicion=$condicion AND punto_cuenta.cedula_usuario = usuarios.cedula_usuario AND punto_cuenta.condicion<>0 order by punto_cuenta.id_punto DESC";
    ...
    }elseif($total_puntos==0){
        $query="SELECT * FROM punto_cuenta, usuarios WHERE date_part('year',punto_cuenta.fecha_punto)= '$year' AND punto_cuenta.condicion=$condicion AND punto_cuenta.cedula_usuario='$cedula_usuario' AND punto_cuenta.cedula_usuario = usuarios.cedula_usuario AND punto_cuenta.condicion<>0 order by punto_cuenta.id_punto  DESC LIMIT 10 OFFSET '$page'";
        $result = pg_query($query)or die(pg_last_error());

        $query2="SELECT * FROM punto_cuenta, usuarios WHERE date_part('year',punto_cuenta.fecha_punto)= '$year' AND punto_cuenta.condicion=$condicion AND punto_cuenta.cedula_usuario='$cedula_usuario' AND punto_cuenta.cedula_usuario = usuarios.cedula_usuario AND punto_cuenta.condicion<>0 order by punto_cuenta.id_punto DESC";
    ...
    $query="SELECT * FROM punto_cuenta, usuarios WHERE date_part('year',punto_cuenta.fecha_punto)= '$year' AND punto_cuenta.cedula_usuario='$cedula_usuario_a' AND punto_cuenta.cedula_usuario = usuarios.cedula_usuario AND punto_cuenta.condicion<>1 order by punto_cuenta.id_punto DESC LIMIT 10 OFFSET '$page'";
    $result = pg_query($query)or die(pg_last_error());

    $query2="SELECT * FROM punto_cuenta, usuarios WHERE date_part('year',punto_cuenta.fecha_punto)= '$year'  AND punto_cuenta.cedula_usuario='$cedula_usuario_a' AND punto_cuenta.cedula_usuario = usuarios.cedula_usuario AND punto_cuenta.condicion<>1 order by punto_cuenta.id_punto DESC";
    $datos_consultaT = pg_query($query2) or die(pg_last_error());
        ...
        <div class="panel panel-default">
            ...
            <div class="panel-heading">
                ...
                <div style="float:right;">
                    ...
                    echo '<a href="index2.php?view=punto_cuenta_add" class="btn btn-default btn-primary">Agregar</a>';
                    };?>
                </div>
                <h4 class="text-primary"><strong> REGISTRO DE PUNTOS DE CUENTAS </strong></h4> 
            </div>
            ...
            <form name="punto_cuenta_load" method="POST" action="" enctype="multipart/form-data"> 
                <div class="col-lg-4">              
                    <div class="form-group">
                        ...
                        <select name="year" id="year" onchange="javascript: submit_punto_cuenta_load();" class="form-control">
                            <?php 
                            $consulta_sql=pg_query("SELECT date_part('year',fecha_punto) AS year FROM punto_cuenta  group by date_part('year',fecha_punto) order by date_part('year',fecha_punto) DESC");                                
                ...
                <div class="col-lg-4">
<?php if(($_SESSION['nivel']==0)||($result_permisos[solicitar_punto]==1)){ ?>
                    <div class="form-group">
                        ...
                        <select name="cedula_usuario" id="cedula_usuario" class="form-control" onchange="javascript: submit_punto_cuenta_load();">
                            ...
                            <?php 
                                $consulta_sql=pg_query("SELECT * FROM usuarios where usuarios.solicitar_punto=1 order by usuarios.nombre_usuario");
                                while ($array_consulta=pg_fetch_array($consulta_sql)){
...
<?php }?>
                ...
                <div class="form-group col-lg-4">
                    <label>ESTADO DEL PUNTO DE CUENTA:</label>
                    <select name="condicion" id="condicion" class="form-control" onchange="javascript: submit_punto_cuenta_load();" >
                        <?php
                            if($condicion=="1") {
                                echo '<option value="1" selected="selected">Pendientes</option>';
                                echo '<option value="2" >Procesados</option>';
                            }elseif($condicion=="2") {
                                echo '<option value="1" >Pendiente</option>';
                                echo '<option value="2" selected="selected">Procesados</option>';
                            }else{
                                echo '<option value="1" >Pendiente</option>';
                                echo '<option value="2" >Procesados</option>';
                            }                                                                               
