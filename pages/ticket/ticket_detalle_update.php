<?php
    ...
    if (isset($_GET['cod_detalle_ticket'])){
        $cod_detalle_ticket=strtoupper($_GET['cod_detalle_ticket']);
        
        $query="SELECT * FROM ticket, detalles_ticket,tramites,solicitantes,estados_tramites,unidades,categorias". 
                " WHERE detalles_ticket.cod_detalle_ticket='$cod_detalle_ticket' AND ticket.cod_ticket=detalles_ticket.cod_ticket AND ticket.cod_estado_tramite=estados_tramites.cod_estado_tramite ".
    ...
    if (isset($_POST[save])){   // Insertar Datos del formulario
        $cod_detalle_ticket=strtoupper($_POST['cod_detalle_ticket']);
...
//        $hora_desde=date("H:i", mktime($thorad,$tminutosd,0));
        ...
        $sms=$resultados_empresa[sms_completar_ticket];
        // FIN CONSULTA DE EMPRESA

       
        $error="bien";
        $query="update detalles_ticket set cod_ticket='$cod_ticket', cod_estado_tramite='$cod_estado_tramite', cod_unidad='$cod_unidad',fecha_atencion='$fecha_atencion', hora_atencion='$hora_atencion', situacion_actual='$situacion_actual', actuacion='$actuacion', monto_autorizado='$monto_autorizado', observaciones='$observaciones', user_login='$user' where cod_detalle_ticket='$cod_detalle_ticket'";
        $result = pg_query($query)or die(pg_last_error());
        ...
        if ($result_insert_detalle[0]){
            $query="update ticket set cod_estado_tramite='$cod_estado_tramite', cod_subticket='$cod_subticket', respuesta='$respuesta' where cod_ticket='$cod_ticket'";
            $result = pg_query($query)or die(pg_last_error());
    ...
    <div class="col-lg-12">
        <div class="panel panel-default">
            ...
            <div class="panel-heading">
                ACTUALIZAR ACTUACION DE COMPLETAR TICKET
            </div>
            ...
            <div class="panel-body">
                <div class="row">
                    <form method="POST" action="<?php echo $pagina?>" id="QForm" name="QForm" enctype="multipart/form-data">
                        <input class="inputbox" type="hidden" id="cod_detalle_ticket" name="cod_detalle_ticket" value="<?php echo $resultados_subticket['cod_detalle_ticket']; ?>"/>
                        ...
                        <div class="col-lg-6">
                            ...
                            <div class="form-group">
                                ...
                                <textarea class="form-control"  name="situacion_actual" id="situacion_actual" cols="84" rows="2"  >SE COMPLETO CON EXITO LA SOLICITUD</textarea>
                            </div>

                            <div class="form-group">
                                <label>ACTUACIÓN</label>
                                <textarea class="form-control" placeholder="ESCRIBA AQUI LAS RAZONES POR LA QUE SE HACE NECESARIO PROGRAMAR CITA PARA LA ATENCIÓN DEL REQUERIMIENTO"   name="actuacion" id="actuacion" rows="2" ><?php echo $resultados_ticket['descripcion_ticket']?></textarea> 
                            </div>
                        </div>
                        ...
                        <div class="col-lg-12">
                            <div class="form-group">
                                ...
                                <textarea  name="observaciones" id="observaciones" class="form-control" rows="2" ><?php echo $resultados_subticket[observaciones];?></textarea> 
                            </div>
                            <button type="submit" name="save" class="btn btn-default">Enviar</button>
                            <input  class="btn btn-default" type="button" onclick="javascript:window.location.href='?view=gestion_tickets&cod_ticket=<?php echo $resultados_ticket['cod_ticket'];?>'" value="Cerrar" name="cerrar" />
                        </div>
                    </form>
                </div>
            </div>
