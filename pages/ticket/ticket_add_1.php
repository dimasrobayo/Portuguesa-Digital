<?php
    ...
    if (isset($_POST['cedula_rif'])){ // Recibir los Datos 
        ...
        $query="select * from solicitantes,comunidades,ticket where ticket.cedula_rif=solicitantes.cedula_rif and solicitantes.cedula_rif='$cedula' AND solicitantes.idcom=comunidades.idcom";
    ...
    if (isset($_POST[save])){   // Insertar Datos del formulario
        ...
        if ($result_solicitantes[1]){
            $query="SELECT update_solicitante_ticket('$cedula',$cod_tipo_solicitante,'$nombreapellido','$sexo','$fecha_nac',$direccion,'$telefono','$celular','$email',$codcom,$empleado_publico,'$ente_publico',$miembro_partido,'$nombre_partido','$profesion_solicitante',$miembro_clp,$miembro_ubch,$miembro_umujer,$miembro_francisco,$miembro_mincomuna,$pregonero,now())";
            $result = pg_query($query)or die(pg_last_error());
            ...
            if ($result_insert[0]){
                ...
                if ($result_insert_detalle[0]){
                    ...
                    $result = pg_query($query)or die(pg_last_error('1'));
                }
