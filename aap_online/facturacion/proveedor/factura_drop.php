...
if (isset($_GET['cedula_solicitante'])){
	$datos_borrar= $_GET['cedula_solicitante'];

	//se le hace el llamado al archivo de conexion y luego se realiza el enlace.	
	require("conexion/aut_config.inc.php");
	$db_conexion=pg_connect("host=$sql_host dbname=$sql_db user=$sql_usuario password=$sql_pass");	

	$consulta = pg_query("SELECT * FROM solicitantes") or die(pg_last_error());
	...
	else 
	{
		...
		$result_borrar=pg_query("SELECT drop_solicitantes($datos_borrar)") or die(pg_last_error());
		pg_close();
	}
}
?> 
