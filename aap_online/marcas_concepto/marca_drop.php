...
//        echo "<script type='text/javascript'>window.location.href='index.php'</script>";
	...
	$datos_borrar= $_GET[codigo_marca];
        
        $query="SELECT * FROM marca_concepto WHERE marca_concepto.codigo_marca=$datos_borrar AND marca_concepto.codigo_marca IN (SELECT codigo_marca FROM vehiculo_solicitante WHERE vehiculo_solicitante.codigo_marca =$datos_borrar)";			
        ...
        if ($total_registros == 1){
            ...
            $error="bien";	
            $result_borrar=pg_query("DELETE FROM marca_concepto WHERE codigo_marca=$datos_borrar") or die(pg_last_error());
            pg_close();
        }
    }
?> 
