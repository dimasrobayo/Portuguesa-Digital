...
//        echo "<script type='text/javascript'>window.location.href='index.php'</script>";
    ...
    if (isset($_GET[status])){
        ...
        echo $codigo = $_GET[codigo_marca];

        if($status == 1) {
            $status_update = 0;
            $resultado=pg_query("UPDATE marca_concepto SET status=$status_update WHERE codigo_marca=$codigo;") or die(pg_last_error());
            pg_close();	
            $error="bien";
        } else {
            $status_update = 1;
