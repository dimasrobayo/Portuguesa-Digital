<?php
    ...
    if (isset($_POST['codmun'])){
        ...
        $parroquia=$_POST["codpar"];
        $comunidad=$_POST["codcom"];
        $year=$_POST["year"];
    }
    ...
    $pdf->SetY(20);
    $pdf->Cell(150,6,utf8_decode('LISTADO DE SOLICITUDES POR COMUNIDAD'),0,1,'C',0);//
    
    $query_comunidad="SELECT descom FROM comunidades WHERE codmun='$municipio' and codest='$estado' and codpar='$parroquia' and idcom='$comunidad'";              
    $result_comunidad = pg_query($query_comunidad)or die(pg_last_error());  
    $resultados_comunidad=pg_fetch_row($result_comunidad);  
    pg_free_result($result_comunidad);

    $nombre_comunidad=$resultados_comunidad[0];
    
    $pdf->SetY(32);
    $pdf->SetFont('Times','B',12);
    $pdf->MultiCell(0,6,'ANO DE REGISTROS: '.implode('/',array_reverse(explode('-',$year))).' COMUNIDAD: '.implode('/',array_reverse(explode('-',$nombre_comunidad))),0,'R',0);//
    ...
    $query="SELECT *, ticket.fecha_registro AS fecha_registro_ticket FROM ticket,tramites,solicitantes,estados_tramites,unidades,categorias,estados,municipios,parroquias,comunidades WHERE  date_part('year',ticket.fecha_registro)=$year AND comunidades.codmun='$municipio' AND comunidades.codest='$estado' AND comunidades.codpar='$parroquia' AND comunidades.idcom='$comunidad' AND  ticket.cod_estado_tramite=estados_tramites.cod_estado_tramite AND ticket.cedula_rif=solicitantes.cedula_rif AND ticket.cod_tramite=tramites.cod_tramite  AND tramites.cod_unidad=unidades.cod_unidad and tramites.cod_categoria=categorias.cod_categoria and solicitantes.idcom=comunidades.idcom and comunidades.codest=estados.codest and comunidades.codmun=municipios.codmun and comunidades.codpar=parroquias.codpar and parroquias.codest=estados.codest and parroquias.codmun=municipios.codmun and municipios.codest=estados.codest order by ticket.cod_ticket";
