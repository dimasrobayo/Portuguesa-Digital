<?php
    ...
    $pdf->SetY(20);
    $pdf->Cell(150,6,utf8_decode('LISTADO DE SOLICITUDES POR UNIDAD'),0,1,'C',0);//
    
    $query_unidad="SELECT nombre_unidad FROM unidades WHERE cod_unidad=$unidad";              
    $result_unidad = pg_query($query_unidad)or die(pg_last_error());  
    $resultados_unidad=pg_fetch_row($result_unidad);  
    pg_free_result($result_unidad);

    $nombre_unidad=$resultados_unidad[0];
    
    $pdf->SetY(32);
    $pdf->SetFont('Times','B',12);
    $pdf->MultiCell(0,6,'ANO DE REGISTROS: '.implode('/',array_reverse(explode('-',$year))).' UNIDAD: '.implode('/',array_reverse(explode('-',$nombre_unidad))),0,'R',0);//
    ...
    $query="SELECT *, ticket.fecha_registro AS fecha_registro_ticket FROM ticket,tramites,solicitantes,estados_tramites,unidades, categorias WHERE  date_part('year',ticket.fecha_registro)=$year AND unidades.cod_unidad=$unidad AND  ticket.cod_estado_tramite=estados_tramites.cod_estado_tramite AND ticket.cedula_rif=solicitantes.cedula_rif AND ticket.cod_tramite=tramites.cod_tramite  AND tramites.cod_unidad=unidades.cod_unidad and tramites.cod_categoria=categorias.cod_categoria order by ticket.cod_ticket";
