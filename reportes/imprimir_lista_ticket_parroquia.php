<?php
    ...
    $query_parroquia="SELECT despar FROM parroquias WHERE codmun='$municipio' and codest='$estado' and codpar='$parroquia'";              
    $result_parroquia = pg_query($query_parroquia)or die(pg_last_error());  
    $resultados_parroquia=pg_fetch_row($result_parroquia);  
    pg_free_result($result_parroquia);

    $nombre_parroquia=$resultados_parroquia[0];
    
    $pdf->SetY(32);
    $pdf->SetFont('Times','B',12);
    $pdf->MultiCell(0,6,'ANO DE REGISTROS: '.implode('/',array_reverse(explode('-',$year))).' PARROQUIA: '.implode('/',array_reverse(explode('-',$nombre_parroquia))),0,'R',0);//
