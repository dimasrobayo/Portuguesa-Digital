<?php
    ...
    $pdf->SetY(20);
    $pdf->Cell(150,6,utf8_decode('LISTADO DE SOLICITUDES POR CATEGORIA'),0,1,'C',0);//
    
    $query_categoria="SELECT descripcion_categoria FROM categorias WHERE cod_categoria=$categoria";              
    $result_categoria = pg_query($query_categoria)or die(pg_last_error());  
    $resultados_categoria=pg_fetch_row($result_categoria);  
    pg_free_result($result_categoria);

    $descripcion_categoria=$resultados_categoria[0];
    
    $pdf->SetY(32);
    $pdf->SetFont('Times','B',12);
    $pdf->MultiCell(0,6,'ANO DE REGISTROS: '.implode('/',array_reverse(explode('-',$year))).' CATEGORIA: '.implode('/',array_reverse(explode('-',$descripcion_categoria))),0,'R',0);//
    ...
    $query="SELECT *, ticket.fecha_registro AS fecha_registro_ticket FROM ticket,tramites,solicitantes,estados_tramites,unidades, categorias WHERE  date_part('year',ticket.fecha_registro)=$year AND tramites.cod_categoria=$categoria AND  ticket.cod_estado_tramite=estados_tramites.cod_estado_tramite AND ticket.cedula_rif=solicitantes.cedula_rif AND ticket.cod_tramite=tramites.cod_tramite  AND tramites.cod_unidad=unidades.cod_unidad and tramites.cod_categoria=categorias.cod_categoria order by ticket.cod_ticket";
