<?php
    ...
    if ($resultados_ticket[cod_unidad]==5) {
        $pdf->Cell(110,6,utf8_decode('TICKET DE ATENCIÓN AL TRANSPORTISTA '),0,0,'C',1);//
    }else{
        $pdf->Cell(110,6,utf8_decode('TICKET DE ATENCIÓN AL PORTUGUESEÑO '),0,0,'C',1);//
    }
...
//    $pdf->SetFontSize(9);
    ...
    $pdf->SetFont('Times','B',9);
    $pdf->Cell(75,4,utf8_decode('PERSONA DE CONTACTO'),1,0,'C',0);
	...
	$pdf->Cell(75,5,utf8_decode($resultados_ticket[persona_contacto_dep]),1,0,'L',0);
    ...
    $pdf->Cell(55);
    $pdf->SetFont('');
    $pdf->Cell(30,5,utf8_decode('Monto Autorizado:'),0,0,'L',0);
    $pdf->SetFont('','B');
    $pdf->Cell(20,5,utf8_decode($resultados_detalle_ticket['monto_autorizado']),0,0,'L',0);
    ...
    $pdf->SetFontSize(8);
    $pdf->SetFont('');
    $pdf->Cell(0,4,'Impreso por: '.$user.' el '.date("d/m/Y g:i A."),0,1,'R',0);
    ...
    if($resultados_ticket['prioridad_ticket']==1){
        $pdf->Image('./logo/nivel_1.png',80,123,'11','','png','');
    }elseif($resultados_ticket['prioridad_ticket']==2){
        $pdf->Image('./logo/nivel_2.png',80,123,'11','','png','');
    }else{
        $pdf->Image('./logo/nivel_3.png',80,123,'11','','png','');
    }
    ...
    if ($resultados_ticket[cod_unidad]==5) {
        ...
        $pdf->SetFont('');
        $pdf->Cell(85);
        $pdf->MultiCell(0, 3, utf8_decode('Depositar (solo efectivo) en el Bando de Venezuela a nombre de BusPortuguesa, S.A.      Cuenta corriente 0102-0422-43-0000254898.'),1, 'J',0,0);
    }
    
    ////// MITAD DE PAGINA
    
    $pdf->SetFont('Arial','B',16);
    $pdf->Image('./logo/logo.jpg',10,137,'32','','jpg','http://www.alcaldiaguanare.gob.ve');
    ...
    if($resultados_ticket['prioridad_ticket']==1){
        $pdf->Image('./logo/nivel_1.png',80,250,'11','','png','');
    }elseif($resultados_ticket['prioridad_ticket']==2){
        $pdf->Image('./logo/nivel_2.png',80,250,'11','','png','');
    }else{
        $pdf->Image('./logo/nivel_3.png',80,250,'11','','png','');
    }
